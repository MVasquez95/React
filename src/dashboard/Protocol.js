...
import { fetchProtocols, updateProtocol, createProtocol, deleteProtocol } from '../calls/protocol'
...
const ProtocolController = () => {
  ...
  const reducer = (model, msg) => {
    switch (msg.type) {
      ...
      case 'FETCH_PROTOCOLS':
        return { ...model, protocols: msg.payload, mode: 'view', formData: { ...entityModel } };
      // References
  ...
  const middlewareUpdate = update => action => {
    switch (trace(action.type)) {
      case 'CREATE_PROTOCOL':
        ...
        createProtocol(action.payload) // { schema }
          .then(res => trace(res))
          .then(_ => fetchProtocols({}))
          .then(protocols => update({ type: 'FETCH_PROTOCOLS', payload: protocols }))
          .catch(_ => update({ type: 'ERROR_MODE' }))
        break
      case 'UPDATE_PROTOCOL':
        update({ type: 'LOADING_MODE' })
        updateProtocol(action.payload["_id"], action.payload["data"])
          .then(_ => fetchProtocols({}))
          .then(protocols => update({ type: 'FETCH_PROTOCOLS', payload: protocols }))
          .catch(_ => update({ type: 'ERROR_MODE' }))
        break
      case 'DELETE_PROTOCOL':
        update({ type: 'LOADING_MODE' })
        deleteProtocol(action.payload) // { _id: "a221asd..." }
          .then(_ => fetchProtocols({}))
          .then(protocols => update({ type: 'FETCH_PROTOCOLS', payload: protocols }))
          .catch(_ => update({ type: 'ERROR_MODE' }))
        break
  ...
  return <Protocol {...{ model, update: middlewareUpdate(update) }} />
}
...
const View = ({ model, update }) => <Column className="flex-1 overflow-x-hidden overflow-y-auto w-full h-full p-10">
  ...
  <Table columns={zipSchema(model.schema).filter(([_, metadata]) => metadata.visible === true).map(([p, _]) => p)}>
    {model.protocols.map(protocol =>
      <tr key={protocol._id} className="bg-white hover:bg-gray-300 shadow-inner">
        {zipSchema(model.schema).filter(([_, metadata]) => metadata.visible === true).map(([p, metadata], i) => // use schema here
          <td key={i} className="border px-4 py-2 cursor-pointer "
            onClick={() => update({ type: 'EDIT_MODE', payload: protocol })}>
            {metadata.ref ? protocol[p].name : null}
            {metadata.type === 'String' ? (protocol[p] ? protocol[p].toString(): 'Not Found!') : null}
            {metadata.type === 'Date' ? formatDate(new Date(protocol[p])) : null}
            {metadata.type === 'Number' ? protocol[p].toString() : null}
          </td>)}
        <td className="px-4 py-2 bg-gray-400 hover:bg-gray-400">
          <DeleteButton onClick={() => update({ type: 'DELETE_PROTOCOL', payload: protocol._id })} />
        </td>
      </tr>)}
  </Table>
</Column>

const Create = ({ model, update }) => <FormModal title="Crear Protocol">
  <Button key="back" onClick={() => update({ type: 'VIEW_MODE' })}>Volver</Button>
  <Form key="form" {...{ model, update }} />
  <Button key="submit" onClick={() => update({ type: 'CREATE_PROTOCOL', payload: model.formData })}>Agregar</Button>
</FormModal>

const Update = ({ model, update }) => <Row className="w-full center-x">
  <FormModal title="Editar Protocol">
    <Button key="back" onClick={() => update({ type: 'VIEW_MODE' })}>Volver</Button>
    <Form key="form" {...{ model, update }} />
    <Button key="submit" onClick={() => update({
      type: 'UPDATE_PROTOCOL', payload: {
