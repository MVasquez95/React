...
import { fetchClassifiers, updateClassifier, createClassifier, deleteClassifier } from '../calls/classifier'
...
const ClassifierController = () => {
  ...
  const reducer = (model, msg) => {
    switch (msg.type) {
      ...
      case 'FETCH_CLASSIFIERS':
        return { ...model, classifiers: msg.payload, mode: 'view', formData: { ...entityModel } };
      // References
  ...
  const middlewareUpdate = update => action => {
    switch (trace(action.type)) {
      case 'CREATE_CLASSIFIER':
        ...
        createClassifier(action.payload) // { schema }
          .then(res => trace(res))
          .then(_ => fetchClassifiers({}))
          .then(classifiers => update({ type: 'FETCH_CLASSIFIERS', payload: classifiers }))
          .catch(_ => update({ type: 'ERROR_MODE' }))
        break
      case 'UPDATE_CLASSIFIER':
        update({ type: 'LOADING_MODE' })
        updateClassifier(action.payload["_id"], action.payload["data"])
          .then(_ => fetchClassifiers({}))
          .then(classifiers => update({ type: 'FETCH_CLASSIFIERS', payload: classifiers }))
          .catch(_ => update({ type: 'ERROR_MODE' }))
        break
      case 'DELETE_CLASSIFIER':
        update({ type: 'LOADING_MODE' })
        deleteClassifier(action.payload) // { _id: "a221asd..." }
          .then(_ => fetchClassifiers({}))
          .then(classifiers => update({ type: 'FETCH_CLASSIFIERS', payload: classifiers }))
          .catch(_ => update({ type: 'ERROR_MODE' }))
        break
  ...
  return <Classifier {...{ model, update: middlewareUpdate(update) }} />
}
...
const View = ({ model, update }) => <Column className="flex-1 overflow-x-hidden overflow-y-auto w-full h-full p-10">
  ...
  <Table columns={zipSchema(model.schema).filter(([_, metadata]) => metadata.visible === true).map(([p, _]) => p)}>
    {model.classifiers.map(classifier =>
      <tr key={classifier._id} className="bg-white hover:bg-gray-300 shadow-inner">
        {zipSchema(model.schema).filter(([_, metadata]) => metadata.visible === true).map(([p, metadata], i) => // use schema here
          <td key={i} className="border px-4 py-2 cursor-pointer "
            onClick={() => update({ type: 'EDIT_MODE', payload: classifier })}>
            {metadata.ref ? classifier[p].name : null}
            {metadata.type === 'String' ? (classifier[p] ? classifier[p].toString(): 'Not Found!') : null}
            {metadata.type === 'Date' ? formatDate(new Date(classifier[p])) : null}
            {metadata.type === 'Number' ? classifier[p].toString() : null}
          </td>)}
        <td className="px-4 py-2 bg-gray-400 hover:bg-gray-400">
          <DeleteButton onClick={() => update({ type: 'DELETE_CLASSIFIER', payload: classifier._id })} />
        </td>
      </tr>)}
  </Table>
</Column>

const Create = ({ model, update }) => <FormModal title="Crear Classifier">
  <Button key="back" onClick={() => update({ type: 'VIEW_MODE' })}>Volver</Button>
  <Form key="form" {...{ model, update }} />
  <Button key="submit" onClick={() => update({ type: 'CREATE_CLASSIFIER', payload: model.formData })}>Agregar</Button>
</FormModal>

const Update = ({ model, update }) => <Row className="w-full center-x">
  <FormModal title="Editar Classifier">
    <Button key="back" onClick={() => update({ type: 'VIEW_MODE' })}>Volver</Button>
    <Form key="form" {...{ model, update }} />
    <Button key="submit" onClick={() => update({
      type: 'UPDATE_CLASSIFIER', payload: {
